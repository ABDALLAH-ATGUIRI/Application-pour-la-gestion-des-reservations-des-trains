...
class Voyages
{
    ...
    {
        $db = Database::connect()->prepare("INSERT INTO voyage (date,depart,arrive,nom_train,price)VALUES(:date,:depart,:arrive,:train,:price)");
    ...
    static public function editVoyage($data)
    {
        ...
        arrive = :arrive,nom_train = :nom_train,price = :price 
        where Id_voyage = :Id");
        $db->bindParam(':date', $data['date']);
        $db->bindParam(':depart', $data['depart']);
        $db->bindParam(':arrive', $data['arrive']);
        $db->bindParam(':nom_train', $data['train']);
        $db->bindParam(':price', $data['price']);
